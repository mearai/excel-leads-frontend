...
export const useAuth = ({ middleware, redirectIfAuthenticated } = {}) => {
    ...
    const { data: user, error, mutate } = useSWR('/api/user', () =>
        axios
            ...
            .catch(error => {
                ...
                if (error.response.status === 401 && error.response.data.message === 'Verification code required') {
                    console.log('error.response')
                    console.log(error.response.data.message === 'Verification code required' )
    ...
    const verifyCode = async ({ setErrors, setStatus, setLoading, ...props }) => {
     ...
     const resendVerification  = async ({ setErrors, setStatus,setMessage, ...props }) => {
        try {
    ...
    useEffect(() => {
        if (window.location.pathname === '/verify-code' && error?.response.data.message === 'Unauthenticated.') {
